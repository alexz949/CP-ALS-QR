...
\label{sec:result}
\response{All experimental results presented in this section are collected on MATLAB R2023b in single-threaded mode on an Intel Xeon Gold 6226R system with 256 GB of RAM, running Ubuntu 20.04.6 LTS. In our experiments we used Tensor Toolbox version 3.6. The Matlab code  for the reproducibility of experimental results is publicly available at:  \url{https://github.com/alexz949/CP-Rounding.git}.}
...
\response{Note that the forward error of the QR-based is dominated by the first term of \cref{eq:qr-ferr}, $O(\kappa \cdot \epsilon_{mach})$, because the residual norm (and therefore $\tan \theta$) is zero in exact arithmetic for this experiment.}
