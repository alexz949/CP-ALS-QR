...
Considering a Khatri-Rao product of three matrices each with $r$ columns and more than $r$ rows, we exploit the following identities, where each computed QR decomposition is denoted using underbraces \response{and $\kron$ and $\kr$ denote matrix Kronecker and Khatri-Rao products, respectively}:
...
\begin{equation}
\label{eq:CP}
\T{X}(i_1,i_2,\dots, i_d) \approx \T{K}(i_1, i_2,\dots, i_d) = \sum^{r}_{j=1} \V{\lambda}(j) \mat{A}_1(i_1,j)\mat{A}_2(i_2,j) \dots \mat{A}_d(i_d,j)
\end{equation}
...
$$\min_{\mat{A}_1,\mat{A}_2, \dots,\mat{A}_d} \sum_{i_1=1}^{n_1} \cdots \sum_{i_d=1}^{n_d} \left(\T{X}(i_1,i_2,\dots, i_d) - \sum^{r}_{j=1} \mat{A}_1(i_1,j)\mat{A}_2(i_2,j) \dots \mat{A}_d(i_d,j) \right)^2. $$
...
Computing $\mat{H}_k$ in line \ref{l:NE-Hada} requires $O(dr^2)$ operations, solving the normal equations in line \ref{l:NE-solve} costs $O(r^3+n_kr^2)$, and computing the Gram matrix in line \ref{l:NE-Gram} costs $O(n_kr^2)$.
