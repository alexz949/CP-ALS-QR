...
\ifpdf
\hypersetup{
  pdftitle={Efficient CP Rounding using Alternating Least Squares with QR Decomposition},
  pdfauthor={Alex Zhang, Bhisham Dev Verma, Jan Van Lent, and Grey Ballard}
}
\fi
...
\usepackage{latexsym,array,delarray,amssymb,epsfig}
\usepackage{amsmath}
\usepackage{listings}
\lstset{
  	basicstyle=\ttfamily,
...
\newcommand{\response}[1]{\textcolor{black}{#1}}
...
The CANDECOMP/PARAFAC or canonical polyadic (CP) decomposition for tensors is a popular tool for analyzing and interpreting latent patterns that may be present in multidimensional data \response{\cite{CC70,Harshman70,Hitchcock27}}. 
...
The normal equations are typically used to solve these linear least squares problems within CP-ALS because of the structure of the coefficient matrix \cite{BK25,kolda2009tensor}. 
...
One drawback of their more stable approach is that the computational cost is exponential in $d$, the number of modes of the tensor.
...
CP-Rounding arises in various applications, including high-dimensional, nonlinear partial differential equations (PDEs) \cite{BVT20}, simulation of quantum algorithms \cite{MY22}, and stochastic PDEs in uncertainty quantification \cite{RDB16}.
In this paper, we propose a new method based on the QR decomposition that is just as numerically stable and reduces the costs from exponential to linear in $d$.


As we describe in \cref{sec:background}, in the context of CP-ALS, the coefficient matrix of each linear least squares problem is a Khatri-Rao product of matrices, which is a column-wise Kronecker product \response{\cite{KR68}}.
\Cref{sec:algo} details the various methods for exploiting the structure of these coefficient matrices, including via the normal equations (see \cref{sec:alg:ne} for more details).
...
Considering a Khatri-Rao product of three matrices each with $r$ columns and more than $r$ rows, we exploit the following identities \response{(derived from \cref{prop:krnkrp} given in \cref{sec:cp})}, where each computed QR decomposition is denoted using underbraces \response{and $\kron$ and $\kr$ denote matrix Kronecker and Khatri-Rao products, respectively}:
...
&= (\M{Q}_1 \kron \M{Q}_2 \kron \M{Q}_3) (\M[\hat]{Q}_{12} \kron \M{I}_r) (\underbrace{\M[\hat]{R}_{12} \kr \M{R}_3}_{\M[\hat]{Q}_{123} \M{R}}).  \label{eq:pw} 
\end{align}
...
In \cref{eq:pw}, $\M[\hat]{Q}_{12}$ is $r^2 \times r$ and $\M{I}_r$ is the $r\times r$ identity matrix.
Finally, we form an explicit Khatri-Rao product of the remaining two triangular factors and compute the last QR decomposition.
...
We exploit the implicit structure when applying its transpose to the target matrix of the least squares problem, as the target matrix has similar structure in the case of CP-Rounding. 
...
We also show that our implementation of CP-ALS using QR is faster than the implementation of CP-ALS in the MATLAB Tensor Toolbox \cite{BK07,TensorToolbox}, which is based on the normal equations, because our implementation uses memoization\response{, a technique of computing and storing temporary quantities to avoid recomputation}.
We observe up to a $4\times$ speedup over the normal equations based CP-ALS on a 9-way tensor.
For the sine-of-sums tensors, we show that the normal equations approach can break down completely in the presence of ill-conditioning, while our method maintains small relative errors.
We also illustrate for synthetic data the benefits of trading off accuracy and running time using reduced working precision: our proposed approach is able to run even faster using single precision while maintaining the same accuracy as the normal equations approach using double precision.
We summarize our conclusions in \cref{sec:conclusion} and discuss future directions based on the proposed method. The source code for the reproducibility of our experimental results is available at: \url{https://github.com/alexz949/CP-Rounding.git}.
...
\begin{equation}
\label{eq:CP}
\T{X}(i_1,i_2,\dots, i_d) \approx \T{K}(i_1, i_2,\dots, i_d) = \sum^{r}_{j=1} \V{\lambda}(j) \mat{A}_1(i_1,j)\mat{A}_2(i_2,j) \cdots \mat{A}_d(i_d,j)
\end{equation}
for all $(i_1,i_2,\dots, i_d) \in \response{[n_1] \times [n_2] \times \dots \times [n_d]}$ where $\mat{A}_k \in \mathbb{R}^{n_k \times r}$ is a factor matrix for each $k \in [d]$ and $\bm{\lambda}\in\mathbb{R}^r$ is a vector of weights. 
...
In the case of CP-format tensors like $\T{K}$, the $k$th mode-wise unfolding \response{denoted by $\mat{K}_{(k)}$} can be expressed in terms of the factor matrices:
$$\mat{K}_{(k)} = \mat{A}_k (\mat{A}_N \odot \dots \odot \mat{A}_{k+1} \odot \mat{A}_{k-1} \odot \dots \odot \mat{A}_1)^\top,$$
where $\odot$ denotes the Khatri-Rao product \cite[Prop. 10.10]{BK25}.

We use the following useful property of Khatri-Rao products \response{and their relation to Kronecker products, denoted by $\kron$}:
\begin{property}[{\cite[Prop. A.22]{BK25}}]
\label{prop:krnkrp}
...
The crux of our proposed approach hinges on changing how we solve a sequence of linear least squares problems within CP-ALS, so we review the basics here.
...
The NE specify a (square) linear system of equations and are derived from setting the gradient of the objective function from \cref{eq:LS} to zero, and they involve the Gram matrix of $\mat{A}$ as well as the product $\mat{B}\mat{A}$: $\mat{X}\mat{A}^\top\mat{A} = \mat{B}\mat{A}$.
...
In the QR decomposition approach, we first compute the compact QR decomposition $\mat{A} = \mat{Q}\mat{R}$ (so that $\mat{Q}\in \mathbb{R}^{m \times n}$) and then solve the  triangular system $\mat{X} \mat{R}^\top = \mat{B}\mat{Q}$.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{|c|c|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{Breakdown of complexity for using NE and QR to solve \cref{eq:LS} for $\mat{A} \in \mathbb{R}^{m \times n}$ and $\mat{B} \in \mathbb{R}^{p \times m}$}
  \label{tab:QR-NE-time}
\end{table}


If $\mat{B}$ has few columns ($p\ll n$), NE is cheaper because of the smaller coefficient in the leading order cost ($mn^2$ versus $2mn^2$). 
...
However, if we use the NE approach, then by \cite[Eq. (19.3)]{trefethen1997numerical} the forward error guarantee is  much looser: 
...
When computing a rank-$r$ CP approximation $\T{K}$ for a $d$-way tensor $\T{X}$, we seek to minimize $\| \T{X} - \T{K} \|^2$, where the tensor norm generalizes the vector 2-norm or the matrix Frobenius norm.
Expressed using \cref{eq:CP}, we have  
%$$\min_{\mat{A}_1,\mat{A}_2, \dots,\mat{A}_d} \| \T{X} - \T{K} \|^2 = \min_{\mat{A}_1,\mat{A}_2, \dots,\mat{A}_d} \sum_{i_1=1}^{n_1} \cdots \sum_{i_d=1}^{n_d} \left(\T{X}(i_1,i_2,\dots, i_d) - \sum^{r}_{j=1} \mat{A}_1(i_1,j)\mat{A}_2(i_2,j) \dots \mat{A}_d(i_d,j) \right)^2. $$
\begin{equation}
\label{eq:objfcnval}
\min_{\mat{A}_1, \dots,\mat{A}_d} \sum_{i_1=1}^{n_1} \cdots \sum_{i_d=1}^{n_d} \left(\T{X}(i_1,\dots, i_d) - \sum^{r}_{j=1} \mat{A}_1(i_1,j)  \cdots \mat{A}_d(i_d,j) \right)^2. 
\end{equation}
However, this is a nonlinear, nonconvex optimization problem and no closed form solution exists. 
We resort to using iterative methods to approximate a solution \cite{BK25,kolda2009tensor}.
...
\response{Each iteration of CP-ALS involves updating all factor matrices, and we call the update of a single factor matrix a subiteration.}

We can solve each \response{problem within a subiteration} directly because it is a linear least squares problem.
In updating the $k$th factor matrix, we expose the linear least squares problem by unfolding $\T{X}$ and $\T{K}$ along the $k$th mode:
\begin{align}
\label{eq:cp-als-sub}
\min_{\mat{A}_k}||\T{X} - \T{K}||^2&= \min_{\mat{A}_k}||\mat{X}_{(k)} - \mat{K}_{(k)}||_F^2 \notag \\ 
&=  \min_{\mat{A}_k}||\mat{X}_{(k)} - \mat{A}_k(\mat{A}_d \odot \dots \odot \mat{A}_{k+1} \odot \mat{A}_{k-1} \odot \dots \odot \mat{A}_1)^\top||_F^2 \notag \\ 
&=  \min_{\mat{A}_k}\|\mat{X}_{(k)}-\mat{A}_k\mat{Z}_k^\top\|_F^2,
\end{align}
where $\mat{Z}_k=\mat{A}_d \odot \dots \odot \mat{A}_{k+1} \odot \mat{A}_{k-1} \odot \dots \odot \mat{A}_1$.
...
All of the algorithms we consider here are based on CP-ALS, but they differ in how they solve the linear least squares subproblems specified in \cref{eq:cp-als-sub}.
\response{More details on CP-ALS can be found in \cite[Chapter 11]{BK25}, and details on the underlying computations and their complexity analysis can be found in \cite[Chapter 3]{BK25}.}
...
That is, we wish to compute a lower-rank representation of the input tensor, sometimes referred to as CP-Rounding.
...
with shorthand notation {$\T{X} = [\![\mat{B}_{1}, \dots ,\mat{B}_{d}]\!]$}.
...
\min_{\mat{A}_k}\|\mat{B}_{k}\mat{Y}_k^\top-\mat{A}_k\mat{Z}_k^\top\|_F^2,
\end{equation}
where $\mat{Y}_k = \mat{B}_d \odot \dots \odot \mat{B}_{k+1} \odot \mat{B}_{k-1} \odot \dots \odot \mat{B}_1$.
...
\response{In all algorithms, we leave the convergence criteria unspecified because it is independent of the per-iteration comparisons among the algorithms.
Typical criteria include a maximum number of iterations and/or (change in) the objective function value (given by \cref{eq:objfcnval}).
We note that computing the objective function value at each iteration can be done cheaply with both NE and QR methods \cite[Section 3.3.1]{MVLB23}, though at some sacrifice to numerical accuracy.}
...
We consider general $d$-way tensors with dimensions $n_1 \times \cdots \times n_d$, and we use $r$ to denote the rank of the CP approximation, which is assumed to be smaller than the tensor dimensions unless otherwise noted.
In the case of CP-format input, we use $s$ to denote the input rank.
\response{To simplify the notation of the} complexity analysis, we use notation for arithmetic average ($\bar n$) as well as geometric mean ($\hat n$) of the tensor dimensions, which means the number of dense input tensor entries is $\hat n^d$ and the number of entries in the CP decomposition is $d\bar n r$.
...
\begin{table}[!ht] 
  ...
  \begin{tabular}{|c|c|c|c|c|c|}
    \hline
    \multicolumn{2}{|c|}{\textbf{Normal Equations} (Algorithm~\ref{alg:cp-als-ne})} & \multicolumn{2}{c|}{\textbf{Explicit QR} (Algorithm~\ref{alg:cp-als-qr})} & \multicolumn{2}{c|}{\textbf{Pairwise QR} (Algorithm~\ref{alg:cp-als-pairwise-qr})} \\
    \hline
    Component                                    & Cost                               & Component                                 & Cost                 & Component                                                & Cost \\
    \hline
    MTTKRP  [Line \ref{l:NE-mttkrp}]                                             & $2d \hat{n}^d r$               & Multi-TTM [Line \ref{l:EXP-TTM}]                                         & $2d\hat n^d r$  & Multi-TTM [Line  \ref{l:pairwise-TTM}]                                     & $2d\hat n^d r$  \\
    Compute Gram [Lines \ref{l:NE-Hada} and \ref{l:NE-Gram}]   & $d \bar{n} r^2 + d^2r^2 $ & QR on factor matrices [Line \ref{l:cp-als-q-exp:updateQR}] & $4d \bar n r^2$ & QR on factor matrices [Line \ref{l:pairwise-factorQR}]                 & $4d \bar n r^2$\\
                                                                                                     &                                         & Compute $\mat{Q}_0$ [Line \ref{l:EXP-QR-R}]                    & $4dr^{d+1}$      & Compute pairwise $\M[\hat]{Q}$ [Line \ref{l:pairwise-QR}] & $4d^2r^4$\\
                                                                                                     &                                         & Apply $\mat{Q}_0$  [Line \ref{l:apply}]                                  & $2d\bar n r^d$  & Apply pairwise $\M[\hat]{Q}$ [Line \ref{l:pair-R-TTM}]       & $2d\bar n r^d$\\
     NE Solve [Line \ref{l:NE-solve}]                                              & $2d\bar{n}r^2 +\frac13 dr^3$                & Back Solve [Line \ref{l:cp-als-q-exp:solve}]                          & $d\bar{n} r^2$   & Back Solve [Line \ref{l:pairwise-solve}]& $d\bar{n} r^2$\\
    \hline
    Leading term & $2d \hat{n}^d r$ & Leading term & $2d \hat{n}^d r$ & Leading term & $2d \hat{n}^d r$ \\
    \hline
  \end{tabular}
  }
  \caption{Per-iteration complexity of CP-ALS algorithms for dense input, reporting only the dominant terms for each component.}
...
\begin{table}[!ht] 
  ...
  \begin{tabular}{|c|c|c|c|c|c|}
    \hline
    \multicolumn{2}{|c|}{\textbf{Normal Equations} (Algorithm~\ref{alg:cp-als-ne-k})} & \multicolumn{2}{c|}{\textbf{Explicit QR} (Algorithm~\ref{alg:cp-als-qr-k})} & \multicolumn{2}{c|}{\textbf{Pairwise QR} (Algorithm~\ref{alg:cp-als-pairwise-qr-kruskal}) } \\
    \hline
    Component & Cost & Component & Cost & Component & Cost \\
    \hline
    MTTKRP [Lines \ref{l:round-ne-rhs} and  \ref{l:NE-K-cross}]              & $4d\bar n rs + d^2rs$ & Multi-TTM [Lines \ref{l:exp-finish-rhs} and \ref{l:EXP-K-update-cross}] & $4d\bar{n} rs$     & Multi-TTM [Lines \ref{l:Pair-K-matmul} and \ref{l:Pair-K-apply}]   & $4d\bar n rs$  \\
    Compute Gram [Lines \ref{l:round-ne-gram} and \ref{l:NE-K-Gram}] & $d\bar n r^2+d^2r^2$ &QR on factor matrices [Line \ref{l:cp-round-qr-updateqr}]                        &  $4d \bar n r^2$  & QR on factor matrices [Line \ref{l:Pair-K-update}]                         & $4d \bar n r^2$\\
                                                                                                               &                                    &Compute $\mat{Q}_0$ [Line \ref{l:cp-round-als-exp-qr}]                          &  $4dr^{d+1}$       & Compute pairwise $\M[\hat]{Q}$ [Line  \ref{l:pair-QR-R}]             & $4d^2r^4$\\
                                                                                                               &                                    &Apply $\mat{Q}_0$ [Line \ref{l:EXP-mttkrp}]                                            &  $2dr^ds$           & Apply pairwise $\M[\hat]{Q}$  [Line \ref{l:pair-QR-Rapply}]           & $2d^2r^3s$\\
   NE Solve [Line \ref{l:NE-K-solve}]                                                      & $2d\bar{n}r^2 +\frac13 dr^3$           & Back Solve [Line \ref{l:cp-round-qr-solve}]                                              &  $d\bar{n} r^2$   & Back Solve [Line \ref{l:Pair-K-solve}]                                            & $d\bar{n} r^2$\\

    \hline
     Leading term & $4d\bar n rs$ & Leading term & $2dr^d(s{+}2r)+4d\bar{n} rs$ & Leading term & $4d\bar n rs$ \\
    \hline
  \end{tabular}
  }
  \caption{Per-iteration complexity of CP-ALS algorithms for CP format input, reporting only the dominant terms for each component.}
...
\begin{equation}
  \label{eq:CP-NE}
 \mat{{A}}_k\mat{Z}_k^\top\mat{Z}_k = \mat{X}_{(k)}\mat{Z}_k.
\end{equation}
...
%\AZ{complexity analysis in the text is for one inner loop and in table it is for one outer loop.}

When we have a dense tensor input, the dominant cost of each subiteration comes from the MTTKRP computation of $\mat{M}_k = \mat{X}_{(k)}\mat{Z}_k$ in line \ref{l:NE-mttkrp}, which has leading order arithmetic cost of $2\hat n^dr$ \response{\cite[Section 3.5]{BK25}}.
Computing $\mat{H}_k$ in line \ref{l:NE-Hada} \response{of \cref{alg:cp-als-ne}} requires $O(dr^2)$ operations, solving the normal equations in line \ref{l:NE-solve} costs $O(r^3+n_kr^2)$, and computing the Gram matrix in line \ref{l:NE-Gram} costs $O(n_kr^2)$.
...
If the input tensor {$\T{X}=[\![\mat{B}_{1}, \dots ,\mat{B}_{d}]\!]$} is already in CP format (see \cref{sec:cp-rounding}), we can compute the MTTKRP more efficiently using the following identity \cite[Section 5.2.6]{BK07}:
{\small
\begin{align*}
 \mat{X}_{(k)}\mat{Z}_k &= \mat{B}_k(\mat{B}_d \odot {\cdots} \odot \mat{B}_{k+1} \odot \mat{B}_{k-1} \odot {\cdots} \odot \mat{B}_1)^\top(\mat{A}_d \odot {\cdots} \odot\mat{A}_{k+1} \odot \mat{A}_{k-1} \odot {\cdots} \odot \mat{A}_1) \\
...
These costs, incurred in lines \ref{l:round-ne-rhs} and \ref{l:NE-K-cross} \response{of \cref{alg:cp-als-ne-k}}, are $2n_krs+(d-2)rs$ and $2n_krs$ in mode $k$, respectively.
Computing the coefficient matrix of the normal equations occurs in lines \ref{l:round-ne-gram} and \ref{l:NE-K-Gram} and has costs $(d-2)r^2$ and $n_kr^2$ in mode $k$.
Solving the normal equations in line \ref{l:NE-K-solve} costs $2n_kr^2 + O(r^3)$.
Thus, the sum of an entire CP-Round-ALS-NE iteration (updating all factor matrices once) is given by %\AZ{Should be $4d\bar n rs$ and we don't need $3d\bar n r^2$?}
\begin{equation*}
{4d\bar{n}rs} + O(d\bar{n}r^2 + d^2rs + d^2r^2 + dr^3).
\end{equation*}
The term $4d\bar{n}rs$ dominates assuming $r<\bar{n}$, for example. 
...
The approach proposed by Minster et al. \cite{MVLB23}, which we refer to as CP-ALS-Explicit-QR, uses the more stable QR decomposition for solving least squares problem instead of the normal equations.
...
%  \label{eq:CP-EXP-QR}
%\end{equation}
To solve the least squares problem, we first apply the Kronecker product of orthonormal factors $\mat{Q}_d \otimes \dots \otimes \mat{Q}_{k+1} \otimes \mat{Q}_{k-1} \otimes \dots \otimes \mat{Q}_1$ to $\mat{X}_{(k)}$ from the right:
$$\mat{Y}_{(k)} = \mat{X}_{(k)}(\mat{Q}_d \otimes \dots \otimes \mat{Q}_{k+1} \otimes \mat{Q}_{k-1} \otimes \dots \otimes \mat{Q}_1),$$
which is equivalent to $\T{Y}=\T{X} \times_1 \mat{Q}_1^{\top} \cdots \times_{k-1} \mat{Q}_{k-1}^{\top}\times_{k+1} \mat{Q}_{k+1}^{\top} \cdots \times_{d} \mat{Q}_{d}^{\top}$, a Multi-TTM computation.
...
For dense input, the computation of each subiteration is dominated by line \ref{l:EXP-TTM}, which is a multiple tensor-times-matrix (Multi-TTM) computation to form $\T{Y}$ with cost
$2\hat{n}^dr+O(\hat{n}^{d-1}r^2 + \cdots + \hat{n}r^d)$ \response{\cite[Section 3.4]{BK25}}, where we assume the Multi-TTM is performed in order of largest mode to smallest (the largest mode is at least the geometric mean).
The cost of the QR of the Khatri-Rao product of triangular factors in line \ref{l:EXP-QR-R} is $4r^{d+1}+O(r^3)$, where we assume $\mat{Q}_0$ is formed explicitly.
The cost of multiplying $\mat{Q}_0$ by $\mat{Y}_{(k)}$ in line \ref{l:apply} is $2n_kr^d$.
Solving the triangular system in line \ref{l:cp-als-q-exp:solve} costs $n_kr^2$, and computing the QR of the factor matrix in line \ref{l:cp-als-q-exp:updateQR} is $4n_kr^2+O(r^3)$, where again we form the orthogonal factor explicitly.
%
...
$$2d\hat{n}^dr + O(d\hat{n}^{d-1}r^2 + \cdots + d\hat{n}r^d + d\bar{n}r^d+ dr^{d+1} ).$$
...
\begin{align*}
& \mat{X}_{(k)}(\mat{Q}_d \otimes \dots \otimes \mat{Q}_{k+1} \otimes \mat{Q}_{k-1} \otimes \dots \otimes \mat{Q}_1) =  \notag\\
&\hspace{3.5cm} \mat{B}_k(\mat{Q}_d^\top\mat{B}_d \odot \cdots \odot \mat{Q}_{k+1}^\top\mat{B}_{k+1} \odot \mat{Q}_{k-1}^\top\mat{B}_{k-1} \odot \cdots \odot \mat{Q}_1^\top\mat{B}_1)^\top.
\end{align*}
...
In the case of CP format input, the costs of lines \ref{l:cp-round-als-exp-qr},\ref{l:cp-round-qr-solve} and \ref{l:cp-round-qr-updateqr} are identical to the corresponding lines in \cref{alg:cp-als-qr}, with combined cost of $4r^{d+1}+5n_kr^2+O(r^3)$ in mode $k$.
The cost of line \ref{l:EXP-mttkrp}, which we implement as an MTTKRP, is $2r^ds$.
The cost of the matrix multiplications in lines \ref{l:exp-finish-rhs} and \ref{l:EXP-K-update-cross} together is $4n_krs$.
Thus, the overall cost of CP-Round-ALS-QR-Exp is % \AZ{Should be $2dr^d(s+2r) + 4d\bar n rs$, but $4dr^{d+1}$ seems like larger in our 7-way experiment? and I'm little confused on $9n_kr^2$}
%$$ 2dr^ds + 4dr^{d+1} + O(d\bar{n}rs+d\bar{n}r^2 + r^3) .$$
$$ 2dr^d(s+2r) + 4d \bar{n} rs + O(d\bar{n}r^2 + r^3) .$$
...
Assuming each factor matrix has more rows than columns, the first phase of the algorithm is the same: we perform QR decompositions on individual factor matrices and exploit \cref{prop:krnkrp}:
...
If a factor matrix has more columns than rows, then its individual QR decomposition and associated computations should be skipped, though we do not consider this optimization here.
...
While the order can be optimized for efficiency, for notational convenience we start from the right: we compute a QR decomposition of an explicit Khatri-Rao product of the last two triangular factors, $\mat{R}_2 \odot \mat{R}_1 = \M[\hat]{Q}_1\M[\hat]{R}_2$, yielding
%$$\small \M{R}_{d-1} \odot \dots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \dots \odot \M[\hat]{Q}_1\M[\hat]{R}_2 = (\M{I}_{r^{d-2}} \kron \M[\hat]{Q}_1)(\M{R}_{d-1} \odot \cdots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \dots \odot \M[\hat]{R}_2).$$
\begin{align*}
 &\M{R}_{d-1} \odot \dots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \dots \odot \M[\hat]{Q}_1\M[\hat]{R}_2 =\notag\\
 &\hspace{4.51cm} (\M{I}_{r^{d-2}} \kron \M[\hat]{Q}_1)(\M{R}_{d-1} \odot \cdots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \dots \odot \M[\hat]{R}_2).
\end{align*} 
...
We note that the Khatri-Rao product of triangular matrices has sparse structure that can be exploited to reduce computation in the QR decomposition by a constant factor (see \cite[Section 3.3.3]{MVLB23} for more details), but we do not consider the optimization here.
...
The cost of the Multi-TTM in line \ref{l:pairwise-TTM} is the same as in \cref{alg:cp-als-qr}: $2\hat{n}^dr+O(\hat{n}^{d-1}r^2 + \cdots + \hat{n}r^d)$.
The permutation in line \ref{l:pairwise-permute} has no arithmetic cost, though in practice it may take significant time due to the high relative cost of memory movement.
The pairwise QR decompositions (line \ref{l:pairwise-QR}) together cost $4(d-2)r^4 + O(dr^3)$ for mode $k$, and applying the pairwise orthogonal factors via TTM (line \ref{l:pair-R-TTM}) costs a total of $2n_kr^d+2n_kr^{d-1}+\cdots+2n_kr^3$ \response{\cite[Section 3.3]{BK25}}.
The triangular solve in line \ref{l:pairwise-solve} costs $n_kr^2$.
The cost of the QR decomposition in line \ref{l:pairwise-factorQR} is $4n_kr^2+O(r^3)$ in mode $k$ (we form the orthonormal factor explicitly).
Thus, the total cost of a full iteration (updating each factor matrix once) of CP-ALS-QR-Imp is
$$ 2d\hat{n}^dr + O(d\bar{n}r^d + d^2r^4 + d\hat{n}^{d-1}r^2). $$
...
&\hspace{3.5cm}(\M{C}_d \odot \cdots \odot \M{C}_{k+1} \odot \M{C}_{k-1} \odot \cdots \odot \M{C}_3 \odot \M[\hat]{Q}_1^\top(\M{C}_2 \odot \M{C}_1))^\top = \\
&\hspace{6.2cm}\M{C}_d \odot \cdots \odot \M{C}_{k+1} \odot \M{C}_{k-1} \odot \cdots \odot \M{C}_3 \odot \M[\hat]{C}_2.
\end{align*}
...
In mode $k$, the cost of the QR in line \ref{l:Pair-K-update} is $4n_kr^2$, and the cost of applying the explicit orthogonal factor to the corresponding input factor matrix in line \ref{l:Pair-K-apply} is $2n_krs$.
Performing the pairwise QR decompositions and applying the explicit orthogonal factors occurs in lines \ref{l:pair-QR-R} and \ref{l:pair-QR-Rapply}.
For each subiteration, the cost of the two operations, which are both performed $d-2$ times, is $4dr^4+2dr^3s+O(r^4)$.
The final multiplication and triangular solve in lines \ref{l:Pair-K-matmul} and \ref{l:Pair-K-solve} together cost $2n_krs+n_kr^2$.
Performing a full iteration and updating each factor matrix once has total cost
$$ 4d\bar{n}rs  + O(d^2r^3s + d\bar{n}r^2). $$
The term $4d\bar{n}rs$ dominates assuming $r^2 < \bar{n}$, for example.
...
\response{All experimental results presented in this section are collected using MATLAB R2023b in single-threaded mode on an Intel Xeon Gold 6226R system with 256 GB of RAM, running Ubuntu 20.04.6 LTS. In our experiments we use Tensor Toolbox version 3.6. The Matlab code  for the reproducibility of experimental results is publicly available at:  \url{https://github.com/alexz949/CP-Rounding.git}.}

\subsection{Sine of Sums Tensor}

To test the accuracy and efficiency of the QR-based method, we use a discretization of the multivariate sine of sums function: $\sin(x_1+\dots+ x_d)$.
Repeatedly using the textbook sine of sums and cosine of sums identities for two variables, we obtain a $2^{d-1}$-term separated representation of the function.
For example, 
\begin{equation*}
\begin{split}
...
\end{split}
\end{equation*}
As described in previous work \response{\cite{BM02,MVLB23,MM05}}, there exists a $d$-term separated representation of the form
\begin{equation}
\label{eq:dtermrep}
...
We adjust the condition number of the coefficient matrix by varying $d$ and the distance between consecutive parameters $\alpha_j$ and $\alpha_{j+1}$, and we fix the discretization parameter $n=50$.
...
However, this loss of accuracy is much worse for the normal equations method, and when the condition number is as large as the reciprocal of the square root of machine precision, it completely breaks down.
For the QR-based methods, the loss of accuracy is much more graceful, and both methods maintain forward error close to the square root of machine precision even when the condition number is on the order of \texttt{1e13}.
This experiment confirms the difference in theoretical guarantees presented in \cref{eq:ne-ferr,eq:qr-ferr}.
\response{Note that the forward error of the QR-based methods is dominated by the first term of \cref{eq:qr-ferr}, $O(\kappa \cdot \epsilon_{mach})$, because $\theta$ is small (the residual is zero in exact arithmetic) and $\eta$ is large for this experiment.}
...
\begin{table}[ht!] 
  ...
  \begin{tabular}{|c|c|c|c|c|c|}
    ...
    $d$  & $|\alpha_j-\alpha_{j+1}|$ & $\kappa(\M{Z}_k)$ & NE Err & QR Exp Err & QR Imp Err \\
    \hline
    3  & \texttt{1e00}  & \texttt{1.54e00}  & \texttt{2.27e-16} & \texttt{3.43e-16} & \texttt{5.44e-16} \\
    3  & \texttt{1e-1} & \texttt{5.61e02}  & \texttt{2.87e-12} & \texttt{8.91e-16} & \texttt{1.47e-15} \\
    5  & \texttt{1e-1} & \texttt{9.02e03}  & \texttt{2.37e-09} & \texttt{5.76e-14} & \texttt{6.32e-16}\\
    3  & \texttt{1e-2} & \texttt{5.37e05}  & \texttt{1.64e-08} & \texttt{2.05e-13} & \texttt{2.71e-14} \\
    5  & \texttt{1e-2} & \texttt{4.66e08}  & \texttt{3.20e-02} & \texttt{8.31e-11} & \texttt{1.37e-14}\\
    3  & \texttt{1e-4} & \texttt{5.37e11} & \texttt{1.43e00} & \texttt{6.00e-10} & \texttt{1.00e-12} \\
    5  & \texttt{1e-3} & \texttt{4.59e13} & \texttt{1.00e00}  & \texttt{6.23e-06} & \texttt{1.99e-08}\\
    \hline
   \end{tabular}
  \caption{Comparison of relative forward error when varying $d$ and $|\alpha_j-\alpha_{j+1}|$ for single least squares problem given by sine of sums.}
  \label{tab:LS_err}
...
Here we observe the exponential dependence on $d$ of the explicit QR method's complexity: as $d$ increases, the time required to solve the least squares problem increases drastically.
...
\begin{figure}[ht!]
  \begin{center}
    \includegraphics[scale = 0.55]{lineplot_p.pdf}
    \caption[Figure]{Comparison of run time for single least squares sine-of-sums problem when varying $d$ for NE, QR Imp, and QR Exp ($n=2000$, $r=d$, $s=2^{d-1}$). \label{fig:LS_problem_line}}
  \end{center}
\end{figure}
...
The second observation is that the time spent in factor-wise QR's is greater than that of computing Gram matrices, as they require more flops and are slightly less efficient.
...
We also note that the linear system solve is noticeably slower for the normal equations method; we conjecture this is because MATLAB is resorting to an SVD to solve a linear system it has detected as ill conditioned.

\begin{figure}[ht!]
  \begin{center}
    \includegraphics[scale = 0.65]{breakdown_p.pdf}
    \caption[Figure]{Breakdown of run time for single least squares sine-of-sums problem when varying $d$ for NE and QR Imp ($n=2000$, $r=d$, $s=2^{d-1}$). \label{fig:LS_problem_breakdown}}
  \end{center}
\end{figure}
...
\begin{figure}[ht!]
  \begin{center}
    \includegraphics[width = \textwidth]{Fig_kt_new.pdf}
    \caption[Figure]{Comparison of per-iteration run time for CP-ALS on sine-of-sums CP Rounding problem when varying $d$ and $n$ for NE, QR Imp, and QR Exp ($r=d$, $s=2^{d-1}$). \label{fig:runtime}}
  \end{center}
\end{figure}
...
\begin{figure}[ht!]
  \begin{center}  
    \includegraphics*[width = \textwidth]{sinsums_acc2.pdf}
    \caption[Figure]{Comparison of CP-ALS convergence behavior of NE and QR Imp on sine-of-sums tensor for varying $d$ and random initializations ($n=10$, $r=d$, $s=2^{d-1}$). \label{fig:error}}
  \end{center}  
\end{figure}

{\color{black}
\subsection{Accuracy and Running Time Tradeoff using Single and Double Precision}
In these experiments, we aim to evaluate the trade-off between accuracy and running time for NE-based and implicit Pairwise QR-based CP-ALS algorithms, using both single and double precision floating-point representations. 
We perform experiments on synthetic tensors where the factor matrices (and their Khatri--Rao products) are ill conditioned. 
The synthetic tensor is generated using the method from~\cite{MVLB23}, with Gaussian noise added to factor matrices of specified condition numbers. 
Each factor matrix $\mat{A}_j$ is created as $\mat{A}_j = \mat{U}_j  \boldsymbol{\Sigma} \mat{V}^T$, where $\mat{U}_j \in \mathbb{R}^{n_j \times R}$ is a random matrix with orthonormal columns, $ \boldsymbol{\Sigma} \in \mathbb{R}^{R \times R}$ is a diagonal matrix with geometrically decreasing singular values, ensuring the condition number of  $\mat{A}_j$ is a predetermined value $\kappa$, and $\mat{V} \in \mathbb{R}^{R \times R}$ is an orthogonal matrix shared by all factor matrices $\mat{A}_j $ for $j = 1, \dots, d $. 
This construction allows us to control the condition number \( \kappa \) of each factor matrix and of the Khatri--Rao product of \( d-1 \) factor matrices. 

%In our experiments, we construct a three-way $10000 \times 10000 \times 10000$ tensor with CP-rank $50$. The tensor is generated with a condition number  $\kappa = 10^{10}$ and a noise level of  $ \eta = 10^{-5}$. We set the maximum number of iterations to $20$ and use a convergence tolerance of   $10^{-15}$ for the relative error. We perform two independent trials. In each trial, a random guess is used for initialization, and all algorithms start with the same initial factor matrices. Figure~\ref{fig:single_precission} summaries our experimental results.


In our experiments, we first generate a three-way CP tensor $\T{Y}$ of size $10000 \times 10000 \times 10000$ with CP-rank 50, using the procedure described above, and set the condition number $\kappa = 10^{10}$. 
To this tensor, we add a noise tensor $\T{N}$ whose factor matrices have $450$ columns and are initialized with i.i.d.~samples from the standard normal distribution with noise level $\eta = 10^{-5}$. 
The formal rank of the resulting CP-tensor  ($\T{X} = \T{Y} + \eta \T{N}$) becomes 500, the sum of the ranks of $\T{Y}$ and $\T{N}$, but it is accurately approximated by a rank-50 CP tensor.
We apply the NE-based and implicit Pairwise QR-based CP-ALS algorithms using both single and double-precision floating-point representations to compress it to rank $50$. 
%We set the maximum number of iterations to $20$ and use a convergence tolerance of   $10^{-15}$ for the relative error. 
 
We use a random guess for initialization, all algorithms start with the same initial factor matrices\response{, and we measure the time and error over 20 iterations}.
%We perform two independent trials. In each trial, a random guess is used for initialization, and all algorithms start with the same initial factor matrices. 
Figure~\ref{fig:single_precission} summarizes our experimental results.
\begin{figure}[ht!]
  \begin{center}  
 \includegraphics*[width=0.98\textwidth]{single_precission_10000_3_updated.pdf}
    \caption[Figure]{Comparison of accuracy and running time of NE and QR Imp based CP-ALS algorithms in single and double precision ($n=10000$, $r=50$, $s=500$). \label{fig:single_precission} }%\AZ{maybe add markers to differentiate 3 coinciding error curves} \GB{maybe run two more trials, and if they are consistent, let's just show one convergence plot and say that it is representative} }
  \end{center}  
\end{figure}

%From Figure~\ref{fig:single_precission}, we observe that the NE-based CP-ALS algorithm in single precision exhibited the shortest running time but suffered from instability, with high relative residual errors and erratic convergence.  The QR-based CP-ALS algorithm in single precision demonstrated slightly higher computational time than the NE single precision, but it is still approximately $1.7 \times$ faster than the NE-based CP-ALS in double precision while maintaining stable performance with consistent accuracy. Therefore, despite a slightly longer execution time compared to the NE single precision method, the QR-based method in single precision offers a good trade-off between stability and efficiency, making it a more reliable and faster alternative to NE double precision for most practical applications.

From Figure~\ref{fig:single_precission}, we observe that running time of NE-based and implicit QR-based CP-ALS algorithms in single precision are comparable and shortest. 
Note that the leading-order costs of each algorithm (as given in \cref{tab:kruskal_its_part}), correspond to the MTTKRP/TTM computations (given in blue).
While the complexity analysis suggests they should be comparable between NE-based and QR-based algorithm, the NE-based implementation we benchmark is that of the Tensor Toolbox, which does not use memoization of the cross products as shown in line \ref{l:NE-K-cross} of \cref{alg:cp-als-ne-k}.
However, the NE-based CP-ALS algorithm in single precision exhibits numerical instability, leading to high relative residual errors and erratic convergence behavior. 
In contrast, the implicit QR-based CP-ALS algorithm in single precision remains numerically stable, resulting in consistent accuracy and stable behavior, similar to its double-precision counterparts. 
The single-precision QR-based algorithm is approximately $2 \times$ faster than either method in double precision. 
These results show that the QR-based method in single precision offers a good trade-off between stability and efficiency and makes it a more reliable and faster alternative to the NE-based method in double precision for many practical applications.
...
Our experiments with a single least squares problem show that if the problem is very ill conditioned (condition number of \texttt{4.6e13}), our algorithm achieves an error that is 8 orders of magnitude smaller than the normal equations approach.
...
We also show that the use of a more stable algorithm enables users to reduce the working precision while maintaining the stability of convergence and accuracy of the final result.
Switching from double to single precision provides another $2\times$ speedup.
...
We would like to consider applications for which the rank $r$ exceeds the individual tensor dimensions, as in \cite{MY22}, applying the optimizations specific to that case and comparing against NE-based methods.
