...
&=  \min_{\mat{A}_k}||\mat{X}_{(k)} - \mat{A}_k(\mat{A}_d \odot \dots \odot \mat{A}_{k+1} \odot \mat{A}_{k-1} \odot \dots \odot \mat{A}_1)^\top||_F \notag \\ 
...
\begin{figure}[ht!]
  \begin{center}
    \includegraphics[scale = 0.6]{lineplot_p.pdf}
...
In our experiments, we first generate a three-way CP tensor $\T{Y}$ of size $10000 \times 10000 \times 10000$ with CP-rank 450, using the procedure described above, and set the condition number $\kappa = 10^{10}$. 
To this tensor, we add a noise tensor $\T{N}$ whose factor matrices are initialized with i.i.d. samples from the standard normal distribution with noise level $\eta = 10^{-5}$. 
...
While the complexity analysis suggests they should be comparable between NE-based and QR-based algorithm, the NE-based implementation we benchmark is that of the Tensor Toolbox, which does not use memoization of the cross products as shown in \cref{l:NE-K-cross} of \cref{alg:cp-als-ne-k}.
