    ...
    \Function{$\{\M{A}_{k}\}=$ CP-ALS-QR-Imp}{$\T{X},r$}
      ...
      \While{not converge}
        \For{$k = 1, \dots, d$}
          ...
          \State $\T{Y} = \Call{Permute}{\T{Y},[1,\dots,k-1,k+1,\dots,d,k]}$ \Comment{$\T{Y}$ is $r{\times} \cdots {\times} r {\times} n_k$} \label{l:pairwise-permute}
          \State $\mat{R} = \mat{R}_1$
          \For{$\ell = 2, \dots, d-1$} 
            \State $[\M[\hat]{Q},\mat{R}] = \Call{QR}{\mat{R}_\ell \odot \mat{R}}$ \Comment{pairwise QR} \label{l:pairwise-QR}
            \State $\T{Y}= \Call{Reshape}{\T{Y},[r^2,r^{d-\ell-1},n_k]}$ \Comment{linearize 1st two  modes}
