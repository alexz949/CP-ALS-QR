...
\author{Alex Zhang, Grey Ballard, and Bhisham Dev Verma}
\date{\today}
\textwidth=16.00cm 
\textheight=26.00cm 
\topmargin=0.00cm
...
In these experiments, we aim to evaluate the trade-off between accuracy and running time for NE-based and implicit Pairwise QR-based CP-ALS algorithms, using both single and double precision floating-point representations. We perform experiments on synthetic tensors where the factor matrices (and their Khatri--Rao products) are ill-conditioned. The synthetic tensor is generated using the method from~\cite{MVLB23}. We create the factor matrices randomly and add Gaussian noise. The factor matrices are designed so we can control their condition number. Each factor matrix $\mat{A}_j$ is created as $\mat{A}_j = \mat{U}_j  \boldsymbol{\Sigma} \mat{V}^T$, where $\mat{U}_j \in \mathbb{R}^{n_j \times R}$ is a random matrix with orthonormal columns, $ \boldsymbol{\Sigma} \in \mathbb{R}^{R \times R}$ is a diagonal matrix with decreasing singular values, ensuring the condition number of  $\mat{A}_j$ is a predetermined value $\kappa$, and $\mat{V} \in \mathbb{R}^{R \times R}$ is an orthogonal matrix shared by all factor matrices $\mat{A}_j $ for $j = 1, \dots, d $. This construction allows us to control the condition number \( \kappa \) of each factor matrix and of the Khatri--Rao product of \( d-1 \) factor matrices. 
...
In our experiments, we first generate a three-way CP-tensor of size $10000 \times 10000 \times 10000$ with CP-rank 50, using the procedure described above, and set the condition number $\kappa = 10^{10}$. Let's denote it by $\T{Y}$. To this tensor, we add a noise tensor (let’s say $\T{N}$) whose factor matrices are initialized with i.i.d. samples from the standard normal distribution with noise level $\eta = 10^{-5}$. The rank of the resulting CP-tensor  ($\T{X} = \T{Y} + \eta \T{N}$) becomes the sum of the ranks of $\T{Y}$ and $\T{N}$ due to the addition of noise. However, its actual rank is $50$. We apply the NE-based and implicit Pairwise QR-based CP-ALS algorithms using both single and double-precision floating-point representations to compress it to its actual rank $50$. We set the maximum number of iterations to $20$ and use a convergence tolerance of   $10^{-15}$ for the relative error. We perform two independent trials. In each trial, a random guess is used for initialization, and all algorithms start with the same initial factor matrices. Figure~\ref{fig:single_precission} summaries our experimental results.
\begin{figure}[ht!]
  \begin{center}  
 \includegraphics*[width=0.98\textwidth]{single_precission_10000_3.pdf}
    \caption[Figure]{Comparison of accuracy and running time of NE and QR Imp based CP-ALS algorithms in single and double precision. \label{fig:single_precission} \AZ{maybe add markers to differentiate 3 coinciding error curves} \GB{maybe run two more trials, and if they are consistent, let's just show one convergence plot and say that it is representative} \GB{add comment somewhere to explain difference in blues is due to memoization (our flop counting for NE undercounts the Tensor Toolbox algorithm}}
  \end{center}  
\end{figure}
...
From Figure~\ref{fig:single_precission}, we observe that running time of NE-based and implicit QR-based CP-ALS algorithms in single precision are comparable and shortest. However, the NE-based CP-ALS algorithm in single precision exhibits instability, leading to high relative residual errors and erratic convergence behavior. In contrast, the implicit QR-based CP-ALS algorithm in single precision remains stable, resulting in consistent accuracy and stable performance, similar to its double-precision counterparts, but with running times approximately $2 \times$ shorter. These results show that the QR-based method in single precision offers a good trade-off between stability and efficiency and makes them a more reliable and faster alternative to the NE-based method in double precision for many practical applications.
...
{\color{red} Do we need to add a line about the findings of single and double precision experiments here ....}
