...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    ...
    \For{$k = 1, \dots, d$}
      ...
      \State $\mat{C}_k = \mat{B}_k^\top\mat{A}_k$ \Comment{update Gram}   \label{l:NE-K-Gram}
    \EndFor
    \EndWhile
    \EndFunction
  \end{algorithmic}
...
The cost when having a Kruskal input is basically the same as dense case. Except for $\mat{X}_{(n)}\mat{Z}_n$ which we only need to do hadamard product requires $O(sn_kr)$ instead of $2\hat{n}^dr$.
...
The computation of QR for factor matrices takes $(d-2)r^2\bar{n}$. 
The QR for Khatri-Rao product of $\mat{R}$ takes $O(r^{d+1})$ overall. 
The cost for doing Multi-TTM will be $\hat{n}^dr$ and $\mat{U}_k = \mat{Y}_{(k)}\mat{Q}_0$ in \cref{l:apply} will be $\bar{n}r^d$.
Overall, the sum of cost for CP-ALS-Explicit-QR will be
$$O((d-2)r^2\bar{n} + r^{d+1} + \hat{n}^dr + \bar{n}r^d)$$
\subsubsection{Kruskal Input}
If we have a Kruskal tensor input, the computation of right hand side will be 
\begin{equation}
  \mat{Y}_{(k)}\mat{Q}_0  = \mat{B}_k(\mat{B}_d^\top\mat{Q}_d \odot \dots \odot \mat{B}_{k+1}^\top\mat{Q}_{k+1} \odot \mat{B}_{k-1}^\top\mat{B}_{k-1} \odot  \dots \odot \mat{B}_1^\top\mat{Q}_1)\mat{Q}_0 \nonumber 
\end{equation}
We can treat $\mat{Q}_0$ as a matricized tensor and we can compute this as MTTKRP and a small matrix multiplication.

\begin{algorithm}[!ht]
  \caption{CP-ALS-explicit-QR-Kruskal}
  \begin{algorithmic}[1]
    \Function{$\{\M{A}_{n}\}=$ CP-ALS-Explicit-QR-Kruskal}{$\T{X},r$}
      ...
      \State Update $\mat{Y}_{(n)}  = \mat{B}_k(\mat{B}_d^\top\mat{Q}_d \odot \cdots \mat{B}_{k+1}^\top\mat{Q}_{k+1} \odot \mat{B}_{k-1}^\top\mat{Q}_{k+1} \cdots \odot \mat{B}_1^\top\mat{Q})$ \Comment{Apply factor Qs} \label{l:EXP-K-Apply}
      \State $\mat{U}_k = \mat{Y}_{(k)}\mat{Q}_0 =\mat{B}_k(\mat{B}_d^\top\mat{Q}_d \odot \cdots \mat{B}_{k+1}^\top\mat{Q}_{k+1} \odot \mat{B}_{k-1}^\top\mat{B}_{k-1} \odot  \cdots \mat{B}_1^\top\mat{Q}_1)\mat{Q}_0 $ \Comment{MTTKRP}
      \State Solve $\mat{\hat{A}_k}\mat{R}_0^\top = \mat{U}_k$ \Comment{via TRMS}
...
In next line the MTTKRP dominates the cost which is $(\hat{n}^dr)$.
...
The dominant cost of Pairwise QR is in \cref{l:Pair-TTM} which is $\hat n^d r$. 
The cost for doing TTM on pairwise Q in \cref{l:pair-R-TTM} will be $\bar{n}(r^d + r^{d-1}+\cdots + r^2)$. 
The time spent on computing pairwise QR on $\mat{R}_d \odot \cdots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \cdots \odot \mat{R}_1$ will be $(d-2)r^4$.
The rest computation holds the same for explicit QR dense case.
Overall, the sum of all costs will be 
$$O(r^2\bar n + r^4 + \hat n^d r + \bar n(r^d + \cdots + r^2))$$
\subsubsection{Kruskal Input}
...
This gives us
\begin{align}
  \mat{Y}_k &= (\mat{Q}_d \otimes \dots \otimes \mat{Q}_{k+1} \otimes \mat{Q}_{k-1} \otimes \dots \otimes \mat{Q}_1)(\mat{B}_{k}(\mat{B}_{d} \odot \dots \odot \mat{B}_{k+1} \odot \mat{B}_{k-1}  \odot \dots \odot \mat{B}_{1})^\top) \nonumber \\
  &= (\mat{Q}_d\mat{B}_d \odot \dots \odot \mat{Q}_{k+1} \mat{B}_{k+1} \odot \mat{Q}_{k-1}\mat{B}_{k-1} \odot \dots \odot \mat{Q}_{1}\mat{B}_{1})^\top\mat{B}_k \nonumber   
\end{align}
On following step, we will apply that set of kronecker $\mat{Q}$ into $\mat{Y}_k$ in similar pairwise way.
\begin{align}
  \mat{Y}_k   &= (\mat{Q}_d\mat{B}_d\odot \dots \odot \mat{Q}_{k+1} \mat{B}_{k+1} \odot \mat{Q}_{k-1}\mat{B}_{k-1} \odot \dots \odot \mat{Q}_{1}\mat{B}_{1}) \nonumber   \\
      &= ((\mat{Q}_d\mat{B}_d \odot \mat{Q}_{d-1}\mat{B}_{d-1}) \odot \dots \odot \mat{Q}_{k+1} \mat{B}_{k+1} \odot \mat{Q}_{k-1}\mat{B}_{k-1} \odot \dots \odot \mat{Q}_{1}\mat{B}_{1})^\top\mat{B}_k \nonumber      \\
      \mat{Y}_k\mat{Q}_{A_{d-1}}  &= (\mat{Q}_{A_{d-1}}(\mat{Q}_d\mat{B}_d \odot \mat{Q}_{d-1}\mat{B}_{d-1}) \odot \dots \odot \mat{Q}_{k+1} \mat{B}_{k+1} \odot \mat{Q}_{k-1}\mat{B}_{k-1} \odot \dots \odot \mat{Q}_{1}\mat{B}_{1})^\top\mat{B}_k \nonumber \\
      & \vdots \nonumber \\
    \mat{U}_k  &= (\mat{Q}_{M}\mat{B}_M)^\top\mat{B}_k \nonumber  
\end{align}
...
\begin{algorithm}
  \caption{CP-ALS-Pairwise-QR-Kruskal}
  ...
  \begin{algorithmic}[1]
    \Function{$\{\M{A}_{k}\}=$ CP-ALS-Pairwise-QR}{$\T{X},r$}
      ...
      \While{not converge}
      \For{$k = 1, \dots, d$}
        \State Update $\mat{Y}_{(n)}  = \mat{B}_k(\mat{B}_d^\top\mat{Q}_d \odot \cdots \mat{B}_{k+1}^\top\mat{Q}_{k+1} \odot \mat{B}_{k-1}^\top\mat{Q}_{k+1} \cdots \odot \mat{B}_1^\top\mat{Q})$ \Comment{Apply factor Qs} \label{l:Pair-K-Apply}
        
        \State $\mat{R}_0 = \mat{R}_1$ ,$\mat{U}_{(k)} = \mat{B}_1$ \Comment{Init}
        \For{$i = 2, \dots, d$} \Comment{pairwise QR}
        \State $[\mat{Q}_0,\mat{R}_0] = \Call{QR}{\mat{R}_0 \odot \mat{R}_i}$ \label{l:pair-QR-R}
        \State Update $\mat{U}_{(k)} = \mat{Q}_0(\mat{U}_k \odot \mat{B}_i)^\top$
        \EndFor
        \State $\mat{U}_{(k)} = \mat{U}_{(k)}\mat{B}_k$
              
      \State Solve $  \mat{\hat{A}}_k\mat{R}_0^\top = \mat{U}_{(k)}$ \Comment{via TRMS}
  ...
  \end{algorithmic}
\end{algorithm}
The cost for computing QR for factor matrices and pairwise QR \cref{l:pair-QR-R} is the same as dense case.
