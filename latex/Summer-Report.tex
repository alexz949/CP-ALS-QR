...
The CANDECOMP/PARAFAC or canonical polyadic (CP) decomposition for multidimensional data, or tensors, is a popular tool for analyzing and interpreting latent patterns that may be present in multidimensional data. 
Basically CP decomposition of a tensor refers to its expression as a sum of $r$ rank-one components and each of them is a vector outer product.
One of the most popular methods used to compute a CP decomposition is the alternating least squares (CP-ALS) approach, which solves a series of linear least squares problems. 
Usually to solve these linear least squares problems, normal equations are used for CP-ALS. 
This approach may be sensitive for ill-conditioned inputs. 
Based on this idea, there are already a more stable approach which is solving the linear least squares problems  using QR decomposition conducted by Minster in \cite{minster2021cp} instead.
However, one drawback for current more stable approach is that computing QR decomposition is exponential in the number of modes of input tensor.
In this paper, we proposed a new modified QR decomposition, which exploits the CP-format tensor, reducing the costs to only polynomial to the number of modes.

For ALS approach in CP decomposition, we are trying to solve least square problem with multiple right hand sides.
Given the mathematical form of this least square problem $\min_X \| \mat{A}\mat{X} - \mat{B}\|_{F}$, where $\mat{B}$ is a matrix instead of a vector.
The normal equation approach computes the gram $\mat{A}^\top\mat{A}$ and solve the equation $\mat{A}^\top\mat{A}\mat{X} = \mat{A}^\top\mat{B}$.
The QR-based method compute QR decomposition for $\mat{A} = \mat{Q}\mat{R}$, and solve the equation $\mat{R}\mat{X} = \mat{Q}^\top\mat{B}$.
QR-based method is more stable because the condition number of this linear system will not increase.
With the new modified QR decomposition for CP-format tensor, the dominant computation will be $\mat{A}^\top\mat{B}$ and $\mat{Q}^\top\mat{B}$ given multiple right hand sides.
We could then see the new modified QR method is more numerically stable and also faster than normal equation.
...
The benchmark for CP-ALS with CP-format input is QR decomposition on $\mat{A}$. In previous approach, it first performs QR decomposition with each factor matrices in $\mat{A}$, and then perform the second QR decomposition with the Khatri-Rao product of $\mat{R}$s derived from QR on factor matrices.
The second QR decomposition is exponential to number of modes because of the formation of Khatri-Rao product explicitly. 
This also increases the cost for doing $\mat{Q}^\top\mat{B}$ in the second times.
With our approach, we choose to take Khatri-Rao product of $\mat{R}$s only two at a time and perform QR decomposition respectively. 
This strict the time for doing QR to only $r^4$, where $r$ is the size of upper triangular matrix $R$. 
...
We also included our performance results by creating an approximation problem through Sine of Sums representation in \cref{sec:result}.
...
We believed it will demonstrate enough difference on accuracy and time on normal equation and QR-based method.  
...
Differ than previous QR-based method, we reduce the exponential time for forming full Khatri-Rao product of $\mat{R}$s from QR decomposition of factor matrices by doing QR with two consecutive $\mat{R}$s at a time. 
The pseudo-code and complexity analysis indicates that with CP-format input, the time for doing QR can be reduced into $r^4$ where $r$ is the rank of input tensor.
We also included the test of performance of our new version of QR-based method.
In single least square problem with multiple right hand side, we observed the runtime for our new QR Imp is around the same as normal equation approach.
But the accuracy is better with ill-condition input.
With real tensor input, we observed with larger dimension, the standard CP-ALS approach is actually slower because of the redundant computation in MTTKRP step.
The tradition CP-ALS approach sometimes cannot approximate the correct CP tensor when the number of modes is too high.
Our Qr-based method still works well in ill-conditioned case. \AZ{This is similar to Introduction}\\
\\
\AZ{I should have one paragraph for futrue work but I'm not familiar with the explanation so I leave it blank.}
