...
Because given the condition number for $\mat{A} = \kappa(\mat{A})$, the condition number of least sqaure problem
lies between $\kappa(\mat{A})$ and $\kappa(\mat{A})^2$. Given the formula from Trefethen,
\begin{equation}
  \frac{||\tilde{x} - x||}{||x||} = O \biggl(\biggl(\kappa + \frac{\kappa^2\tan\theta}{\eta}\biggr)\epsilon_{machine}\biggr)
\end{equation}
where $\kappa = \kappa(\mat{A})$,
However, in NE, we first compute $\mat{A}^\top\mat{A}$ and then perform Cholesky solve. We have increased the 
...
\GB{Give equations (19.2) and (19.3) from Trefethen and Bau as numbered equations.  You'll need to define $\theta$ and $\eta$ for (19.2).  Then you can give a little intuition for why this exists.  Here's how you cite: \cite{trefethen1997numerical}.}


\subsection{CP-ALS}
\subsubsection{Least Sqaure Problem}
