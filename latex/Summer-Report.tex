...
$$\T{X}(i_1,i_2,\dots, i_d) \approx \T{K} = \sum^{r}_{j=1}\mat{A_1}(i_1,j)\mat{A_2}(i_2,j) \dots \mat{A_r}(i_r,j)$$
...
\end{figure}
In which we can represent $\T[]{K}$ in shorthand CP notation,
$$\T{K} = [\![\bm{\lambda};\mat{A}_1,\mat{A}_2, \dots,\mat{A}_r ]\!] $$
where $\mat{A}_k = \big[\V[]{a}_1^{(k)} \dots \V[]{a}_r^{(k)}\big]$.
...
The cost of NE depends on the size of $\mat{A}$ and $\mat{B}$. For $\mat{A} \in \mathbb{R}^{m \times n}$, and $\mat{B} \in \mathbb{R}^{p \times m}$. The ditribution of time for doing NE in least sqaure problem is
\begin{table}[!ht]
  \centering
  \begin{tabular}{|c|c|}
    ...
    \hline
  \end{tabular}
  \caption{Breakdown of time for doing NE to solve least sqaure problem}
  \label{tab:NE-time}
\end{table}
...
For QR factorization, we will first compute the QR decomposition $\mat{A} = \mat{Q}\mat{R}$ and then apply this into \cref{eq:NE}.
\begin{align}
  \mat{X}\mat{A}^\top\mat{A} &= \mat{B}\mat{A} \nonumber \\
  \mat{X}(\mat{Q}\mat{R})^\top(\mat{Q}\mat{R}) &= \mat{B}\mat{A} \nonumber \\
  \mat{X}\mat{R}^\top\mat{Q}^\top\mat{Q}\mat{R} &= \mat{B}\mat{A} \nonumber \\
  \mat{X}\mat{R}^\top &= \mat{B}\mat{A}\mat{R}^{-1} = \mat{B}\mat{Q}  \label{eq:QR}  
\end{align}
...
The most commonly used method for QR decomposition is through Householder triangularization. The following time distribution table is based on this method.
\begin{table}[!ht]
  ...
  \begin{tabular}{|c|c|}
    ...
    Components & Number of flops\\
    \hline
    Compute $\mat{Q}\mat{R} = \mat{A}$ & $2mn^2 - 2/3n^3$ \\
    Compute $\mat{B}\mat{Q}$ & $2mnp$\\
    N/A & \\
    Back solves & $pn^2$ \\
    \hline
  \end{tabular}
  \caption{Breakdown of time for QR factorization to solve least sqaure problem}
  \label{tab:QR-time}
\end{table}
...
Using QR is more numerically stable than NE. Because given the condition number for $\mat{A} = \kappa(\mat{A})$, the condition number of least sqaure problem
lies between $\kappa(\mat{A})$ and $\kappa(\mat{A})^2$. However, in NE, we first compute $\mat{A}^\top\mat{A}$ and then perform Cholesky solve. We have increased the 
condition number by computing $\mat{A}^\top\mat{A}$ to $\kappa(\mat{A})^2$. In this case condition number for least sqaure problem can best be $\kappa(\mat{A})^2$. Using QR is more prefered when condition number is really large (i.e.$1e8$).
...
When doing CP rank-$k$ approximation for a N-way tensor $\T{X}$, we are basically minimizing $\| \T{X} - \tilde{\T{X}} \|$ which 
$$\tilde{\T{X}} = \sum^k_{i-1}\mat{A}_1(i) \circ \mat{A}_2(i) \circ \dots \circ \mat{A}_N(i)$$
However, this is a nonlinear, nonconvex optimization problem and no closed form solution exists. We can only use iterative methods to approximate.
