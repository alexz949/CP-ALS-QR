...
Starting from the left, we compute a QR decomposition of an explicit Khatri-Rao product of two triangular factors: $\mat{R}_d \odot \mat{R}_{d-1} = \M[\hat]{Q}_{d-1}\M[\hat]{R}_{d-1}$.
This yields
$$\M[\hat]{Q}_{d-1}\M[\hat]{R}_{d-1} \odot \mat{R}_{d-2} \odot \dots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \dots \odot \mat{R}_1 = (\M[\hat]{Q}_{d-1} \kron \M{I}_{r^{d-2}})(\M[\hat]{R}_{d-1} \odot \mat{R}_{d-2} \odot \dots \odot \mat{R}_{k+1} \odot \mat{R}_{k-1} \odot \dots \odot \mat{R}_1),$$
and we can repeat the pairwise QR decomposition on $\M[\hat]{R}_{d-1} \odot \mat{R}_{d-2}$, and so on (skipping the $k$th factor), each time reducing the number of triangular factors by one.
After $d-2$ pairwise QR decompositions, we obtain  
$$\mat{Z}_k = (\mat{Q}_d \otimes \dots \otimes \mat{Q}_{k+1} \otimes \mat{Q}_{k-1} \otimes \dots \otimes \mat{Q}_1) (\M[\hat]{Q}_{d-1} \otimes \M{I}_{r^{d-2}})(\M[\hat]{Q}_{d-2} \otimes \mat{I}_{r^{d-3}}) \cdots (\M[\hat]{Q}_{2} \otimes \mat{I}_r) \M[\hat]{Q}_{1}\M{R}.$$

\GB{I will continue to edit starting from here...need to describe multiplying into the RHS (in general)}


\subsubsection{Dense Input}

The sample pseudocode of our proposed approach for dense input is shown in \cref{alg:cp-als-pairwise-qr}.
