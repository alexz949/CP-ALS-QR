...
The table for showing backward error is showed in \cref{tab:LS_error}.
...
\begin{table}[!ht] 
  ...
  \begin{tabular}{|c|c|c|c|c|c|}
    ...
    \hline
    3 &1000& \texttt{1e5} & \texttt{1.28e-08} & \texttt{1.41e-12} & \texttt{1.53e-12} \\
    4 &100 & \texttt{5e7} & \texttt{7.11e-04} & \texttt{1.59e-10} & \texttt{1.55e-10} \\
    5 & 50 & \texttt{4e9} & \texttt{2.68e-01} & \texttt{2.11e-08} & \texttt{1.93e-08} \\
    6 & 25 & \texttt{4e11}& \texttt{1.91e-01} & \texttt{2.35e-06} & \texttt{2.22e-06} \\
    7 & 15 & \texttt{3e13}& \texttt{4.78e-01} & \texttt{3.27e-03} & \texttt{2.99e-04} \\
    8 & 10 & \texttt{2e15}& \texttt{5.54e-01} & \texttt{3.00e-02} & \texttt{2.40e-02} \\
    \hline
  \end{tabular}
...
The accuracy using NE starts leveling off when $K = 4e9$ but QR techniques hold the error small.


Based on the problem setup, we further measured the forward error by varying the distance between $\alpha_j$ and $\alpha_{j+1}$.
...
\begin{table}[!ht] 
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|}
    ...
    \hline
    3 &100 & \texttt{1e0}  & \texttt{1.51e0}  & \texttt{3.64e-16} & \texttt{1.44e-15} & \texttt{5.07e-16} \\
    3 &100 & \texttt{1e-1} & \texttt{5.02e2}  & \texttt{6.12e-14} & \texttt{3.70e-15} & \texttt{5.74e-16} \\
    5 & 50 & \texttt{1e-1} & \texttt{7.38e3}  & \texttt{9.80e-11} & \texttt{9.70e-15} & \texttt{1.85e-15}\\
    3 &100 & \texttt{1e-2} & \texttt{4.77e5}  & \texttt{1.32e-08} & \texttt{5.40e-13} & \texttt{8.99e-15} \\
    5 & 50 & \texttt{1e-2} & \texttt{3.71e8}  & \texttt{3.51e-01} & \texttt{4.87e-12} & \texttt{1.27e-14}\\
    5 & 50 & \texttt{1e-3} & \texttt{3.64e13} & \texttt{1.00e00}  & \texttt{9.74e-09} & \texttt{1.94e-09}\\
    \hline
  \end{tabular}
...
\begin{figure}[!ht]
  ...
  \begin{center}
    ...
    \caption[Figure]{log runtime respect to QR Imp for NE and Exp in different dimensions \GB{no need to make time relative, just take the log (base 10 or base 2), and report the slowdown number with respect to QR Imp at the top of each bar, try to make each QR Imp take around a second so they are comparable across dimensions, also run 3-5 times and take median time}\label{fig:LS_problem_log}}
