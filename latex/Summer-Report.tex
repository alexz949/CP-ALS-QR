...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}
    ...
    \For{$k = 1, \dots, d$}
      ...
      \State Solve $\mat{A}_k \mat{H}_k = \mat{D}_{k}$ for $\mat{A}_k$ \Comment{via Cholesky} \label{l:NE-solve}
      \State $\mat{G}_k = \mat{A}_k^\top\mat{A}_k$, $\mat{C}_k = \mat{B}_k^\top\mat{A}_k$ \Comment{update Gram}   \label{l:NE-Gram}
    \EndFor
  \end{algorithmic}
\end{algorithm}
