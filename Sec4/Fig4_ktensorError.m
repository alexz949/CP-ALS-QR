clear

load('Fig4.mat','als_data')
load('Fig4_2.mat', 'als_data_2')

%% plot iterations vs relative error
%als8 is actually 5

figure,


subplot(1,2,1)
semilogy(1:40,als_data.als5,'-.','Color','#EDD80B','linewidth',1), hold on
semilogy(1:40,als_data.imp5,':','Color','#EDD80B','linewidth',1)

semilogy(1:40,als_data.als7,'-.','Color','#0A71ED','linewidth',1)
semilogy(1:40,als_data.imp7,':','Color','#0A71ED','linewidth',1)
semilogy(1:40,als_data.als10,'-.','Color','#F33E09','linewidth',1)
semilogy(1:40,als_data.imp10,':','Color','#F33E09','linewidth',1)
ylim([1e-15 10])
set(gca,'fontsize',10)
legend('5-way NE','5-way QR Imp','7-way NE','7-way QR Imp','10-way NE','10-way QR Imp','fontsize',6)
%title('Relative Error for Different d (1st trial)') 
xlabel('iteration number')
ylabel('Relative residual error')
title('1st trial')
%%

subplot(1,2,2)
semilogy(1:40,als_data_2.als5,'-.','Color','#EDD80B','linewidth',1), hold on
semilogy(1:40,als_data_2.imp5,':','Color','#EDD80B','linewidth',1)

semilogy(1:40,als_data_2.als7,'-.','Color','#0A71ED','linewidth',1)
semilogy(1:40,als_data_2.imp7,':','Color','#0A71ED','linewidth',1)
semilogy(1:40,als_data_2.als10,'-.','Color','#F33E09','linewidth',1)
semilogy(1:40,als_data_2.imp10,':','Color','#F33E09','linewidth',1)
ylim([1e-15 10])

title('2nd trial')


xlabel('iteration number')
ylabel('Relative residual error')
set(gca,'fontsize',10)
set(gcf,'Units','inches');
screenposition = get(gcf,'Position');
set(gcf,...
    'PaperPosition',[0 0 screenposition(3:4)],...
    'PaperSize',[screenposition(3:4)]);
%saveas(gcf,'sinsums_acc2.pdf')
%change d & n